			...
			{
				{
					...
					strtmpl_delimiter_cause_description_exception = reinterpret_cast<IStringTemplate*>(dtclassobj_strtmpl->CreateObject(GetDynamicTypeThisClassObject<IStringTemplate>(), indexeddatastore_parameters));
					...
					if (jsonmemberit_config_internal_Exception_strtmpl_description == jsonmemberit_config_internal_Exception->value.MemberEnd() || !jsonmemberit_config_internal_Exception_strtmpl_description->value.IsArray()) abort();
					{
						for (const Internal::jsonval_config_internal_t& jsonval_config_internal_Exception_strtmpl_description_element : jsonmemberit_config_internal_Exception_strtmpl_description->value.GetArray()) {
							...
							DynamicTypeClassID dtclassid_exception = DynamicTypeClassIDFromUUIDString_RunTime(
								jsonmemberit_config_internal_Exception_strtmpl_description_element_dtclassid_exception->value.GetString(),
								jsonmemberit_config_internal_Exception_strtmpl_description_element_dtclassid_exception->value.GetStringLength(),
								is_successful
							);
							if (!is_successful) abort();
							is_successful = true;
