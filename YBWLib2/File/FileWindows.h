	...
	public:
		...
		[[nodiscard]] inline virtual IException* GetFileSizeULongLong(unsigned long long* size_ret) const noexcept {
			if (!size_ret) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, GetFileSizeULongLong);
			{
				...
				if (!GetFileSizeEx(this->win32handleholder_file.get(), &largeint_size_file)) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(GetFileSizeEx);
				if (largeint_size_file.QuadPart < 0) return YBWLIB2_EXCEPTION_CREATE_UNEXPECTED_EXCEPTION_EXCEPTION();
		...
		[[nodiscard]] inline virtual IException* SetFileSizeULongLong(unsigned long long size) noexcept {
			{
				...
				{
					LARGE_INTEGER largeint_size_file;
					if (size > LLONG_MAX) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, SetFileSizeULongLong);
					largeint_size_file.QuadPart = size;
					if (!SetFilePointerEx(this->win32handleholder_file.get(), largeint_size_file, nullptr, FILE_BEGIN)) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(SetFilePointerEx);
				}
		...
		[[nodiscard]] inline virtual IException* IsEof(bool* is_eof_ret) const noexcept override {
			if (!is_eof_ret) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, IsEof);
			{
				...
				IException* err = this->GetFileSizeULongLong(&size_file);
				if (err) return err;
				err = this->TellULongLong(&position_file);
				if (err) return err;
		...
		[[nodiscard]] inline virtual IException* SeekFromBeginULongLong(unsigned long long distance) noexcept {
			{
				...
				if (distance > LLONG_MAX) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, SeekFromBeginULongLong);
				largeint_distance.QuadPart = distance;
				if (!SetFilePointerEx(this->win32handleholder_file.get(), largeint_distance, nullptr, FILE_BEGIN)) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(SetFilePointerEx);
			}
			return nullptr;
		}
		...
		[[nodiscard]] inline virtual IException* SeekFromEndULongLong(unsigned long long distance) noexcept {
			{
				...
				::std::unique_lock<LockableObjectToSTLWrapper> unique_lock_this(wrapper_lock_this);
				LARGE_INTEGER largeint_distance;
				if (distance > LLONG_MAX) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, SeekFromEndULongLong);
				largeint_distance.QuadPart = distance;
				if (!SetFilePointerEx(this->win32handleholder_file.get(), largeint_distance, nullptr, FILE_END)) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(SetFilePointerEx);
			}
			return nullptr;
		}
		...
		[[nodiscard]] inline virtual IException* SeekForwardULongLong(unsigned long long distance) noexcept {
			{
				...
				::std::unique_lock<LockableObjectToSTLWrapper> unique_lock_this(wrapper_lock_this);
				LARGE_INTEGER largeint_distance;
				if (distance > LLONG_MAX) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, SeekForwardULongLong);
				largeint_distance.QuadPart = distance;
				if (!SetFilePointerEx(this->win32handleholder_file.get(), largeint_distance, nullptr, FILE_CURRENT)) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(SetFilePointerEx);
			}
			return nullptr;
		}
		...
		[[nodiscard]] inline virtual IException* SeekBackwardULongLong(unsigned long long distance) noexcept {
			{
				...
				LockableObjectToSTLWrapper wrapper_lock_this(this->lock_this);
				::std::unique_lock<LockableObjectToSTLWrapper> unique_lock_this(wrapper_lock_this);
				if (distance > (unsigned long long) - LLONG_MIN) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, SeekBackwardULongLong);
				/// <summary>
		...
		[[nodiscard]] inline virtual IException* TellULongLong(unsigned long long* distance_ret) const noexcept {
			if (!distance_ret) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, TellULongLong);
			{
				...
				largeint_zero.QuadPart = 0;
				if (!SetFilePointerEx(this->win32handleholder_file.get(), largeint_zero, &largeint_position_file, FILE_CURRENT)) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(SetFilePointerEx);
				if (largeint_position_file.QuadPart < 0) return YBWLIB2_EXCEPTION_CREATE_UNEXPECTED_EXCEPTION_EXCEPTION();
		...
		[[nodiscard]] inline virtual IException* Read(void* buf, size_t size_buf) noexcept override {
			...
			if (!buf) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, Read);
			{
				...
				LockableObjectToSTLWrapper wrapper_lock_this(this->lock_this);
				::std::unique_lock<LockableObjectToSTLWrapper> unique_lock_this(wrapper_lock_this);
				if (!this->can_read) return YBWLIB2_EXCEPTION_CREATE_INVALID_CALL_EXCEPTION_CLASS(::YBWLib2::Win32File, Read);
				...
				for (; size_buf > MAXDWORD; reinterpret_cast<uint8_t*&>(buf) += dword_step, size_buf -= dword_step) {
					...
					if (!ReadFile(this->win32handleholder_file.get(), buf, dword_step, &dword_cb_read, nullptr) || dword_cb_read != dword_step) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(ReadFile);
				}
				DWORD dword_cb_read = 0;
				if (!ReadFile(this->win32handleholder_file.get(), buf, size_buf & ~(DWORD)0, &dword_cb_read, nullptr) || dword_cb_read != size_buf) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(ReadFile);
#else
				DWORD dword_cb_read = 0;
				if (!ReadFile(this->win32handleholder_file.get(), buf, size_buf, &dword_cb_read, nullptr) || dword_cb_read != size_buf) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(ReadFile);
#endif
			}
			return nullptr;
		}
		...
		[[nodiscard]] inline virtual IException* Write(const void* buf, size_t size_buf) noexcept override {
			...
			if (!buf) return YBWLIB2_EXCEPTION_CREATE_INVALID_PARAMETER_EXCEPTION_CLASS(::YBWLib2::Win32File, Write);
			{
				...
				LockableObjectToSTLWrapper wrapper_lock_this(this->lock_this);
				::std::unique_lock<LockableObjectToSTLWrapper> unique_lock_this(wrapper_lock_this);
				if (!this->can_write) return YBWLIB2_EXCEPTION_CREATE_INVALID_CALL_EXCEPTION_CLASS(::YBWLib2::Win32File, Write);
#if SIZE_MAX > MAXDWORD
				/// <summary>
				...
				for (; size_buf > MAXDWORD; reinterpret_cast<const uint8_t*&>(buf) += dword_step, size_buf -= dword_step) {
					...
					if (!WriteFile(this->win32handleholder_file.get(), buf, dword_step, &dword_cb_written, nullptr) || dword_cb_written != dword_step) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(WriteFile);
				}
				DWORD dword_cb_written = 0;
				if (!WriteFile(this->win32handleholder_file.get(), buf, size_buf & ~(DWORD)0, &dword_cb_written, nullptr) || dword_cb_written != size_buf) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(WriteFile);
#else
				DWORD dword_cb_written = 0;
				if (!WriteFile(this->win32handleholder_file.get(), buf, size_buf, &dword_cb_written, nullptr) || dword_cb_written != size_buf) return YBWLIB2_EXCEPTION_CREATE_EXTERNAL_API_FAILURE_WITH_LAST_ERROR_EXCEPTION(WriteFile);
#endif
			}
			return nullptr;
		}
